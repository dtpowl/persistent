...
class Block a where
    ...
    blockDocCommentBlock :: a -> Maybe DocCommentBlock
...
instance Block EntityBlock where
    ...
    blockDocCommentBlock = entityBlockDocCommentBlock
...
instance Block ExtraBlock where
    ...
    blockDocCommentBlock = extraBlockDocCommentBlock
...
applyCommentLinesToBlock lines a = blockSetDocCommentBlock dcb $ blockSetMembers commentedMembers a
  where
    startLine = sourceLine $ blockFirstPos a
    headerLines = takeWhile (\(spos, _t) -> sourceLine spos < startLine) lines
    memberLines = dropWhile (\(spos, _t) -> sourceLine spos <= startLine) lines
...
associateCommentLinesWithMembers lines ms@(mh : mt) = do
  ...
  where
    ...
    filteredLines = filter (\(pos, _t) -> Set.notMember (sourceLine pos) membersLinePoses) lines
...
applyCommentLinesToMember lines m = case m of
    MemberBlockAttr a -> MemberBlockAttr $ applyCommentLinesToBlockAttr lines a
    MemberExtraBlock b -> MemberExtraBlock $ applyCommentLinesToBlock lines b

applyCommentLinesToBlockAttr
    :: [(SourcePos, CommentToken)] -> BlockAttr -> BlockAttr
applyCommentLinesToBlockAttr [] a = a
applyCommentLinesToBlockAttr lines a = a{blockAttrDocCommentBlock = dcb}
  where
    ls =
        takeWhile
